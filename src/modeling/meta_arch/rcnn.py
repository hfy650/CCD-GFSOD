...
class GeneralizedRCNN(nn.Module):
    ...
    def _forward_once_(self, batched_inputs, gt_instances=None):
        ...
        features_de_rpn = features_refined

        # features_de_rpn = features#之前的
        if self.cfg.MODEL.RPN.ENABLE_DECOUPLE:
            scale = self.cfg.MODEL.RPN.BACKWARD_SCALE
            features_de_rpn = {k: self.affine_rpn(decouple_layer(features_refined[k], scale)) for k in features_refined}
        proposals, proposal_losses = self.proposal_generator(images, features_de_rpn, gt_instances)

        # 更严格的保护：只有当 targets 真正存在时才调用 label_and_sample_proposals
        if gt_instances is not None:
            # 先做一次安全断言/打印，便于定位
            # （在 production 可注释掉打印）
            # print("DEBUG: gt_instances available, will label_and_sample_proposals")

            try:
                proposals1 = self.roi_heads.label_and_sample_proposals(proposals, gt_instances)
            except Exception as e:
                # 捕获并记录具体错误，避免整个推理流程崩溃（便于定位）
                print("ERROR while calling label_and_sample_proposals:", e)
                proposals1 = None

            # 仅在 proposals1 非空且处于训练模式时更新质心（update_centroids 属于训练行为）
            if proposals1 is not None and self.training:
                try:
                    self.refine.update_centroids(features, proposals1)
                except Exception as e:
                    print("ERROR while updating centroids:", e)
        else:
            # 没有 gt_instances：在推理/测试阶段常见。确保不调用 label/sample/update。
            # print("DEBUG: gt_instances is None -> skip label_and_sample_proposals and centroid update")
            pass


        features_de_rcnn = features_refined
        # features_de_rcnn = features#之前的
        if self.cfg.MODEL.ROI_HEADS.ENABLE_DECOUPLE:
            scale = self.cfg.MODEL.ROI_HEADS.BACKWARD_SCALE
            features_de_rcnn = {k: self.affine_rcnn(decouple_layer(features_refined[k], scale)) for k in features_refined}
