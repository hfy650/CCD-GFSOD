...
#         Compute the softmax cross entropy loss for box classification.
#
#         If `self.dynamic_margin_loss` and `self.roi_feats` are present, use the
#         DynamicSemanticMarginLoss module (injected from ROIHeads). Otherwise
#         fallback to standard cross-entropy.
#         """
#         # log accuracy as before
#         self._log_accuracy()
#
#         # If a dynamic margin loss module has been injected, use it
#         if hasattr(self, "dynamic_margin_loss") and self.dynamic_margin_loss is not None:
#             if not hasattr(self, "roi_feats") or self.roi_feats is None:
#                 raise RuntimeError(
#                     "dynamic_margin_loss was injected but outputs.roi_feats is missing. "
#                     "Set outputs.roi_feats = box_pooled before calling losses()."
#                 )
#             loss = self.dynamic_margin_loss(
#                 self.pred_class_logits,
#                 self.gt_classes,
#                 self.roi_feats,
#                 bg_class_ind=self.pred_class_logits.shape[1] - 1,
#                 external_prototypes=getattr(self, "ema_prototypes", None),
#             )
#             return loss
#
#         # fallback to original CE
#         return F.cross_entropy(
#             self.pred_class_logits, self.gt_classes, reduction="mean"
#         )
#
